...
fetch("../pets.json")
  ...
  .then((allPetsInfo) => {
    const OUR_FRIENDS_CARD_CONTAINER = document.querySelector(
      ".our-friends__cards"
    );
    const OUR_FRIENDS_BTNS = document.querySelectorAll(
      ".our-friends__nav__button"
    );
    let petObjects = [];
    let currentPage = 1;
    ...
    function changePage(event) {
      ...
      document.querySelector(".our-friends__current__button").innerHTML =
        currentPage;
    ...
    function createCards() {
      ...
      for (let i = 0; i < numberOfCards; i++) {
        OUR_FRIENDS_CARD_CONTAINER.appendChild(
          createCard(
            petObjects[currentPage - 1][i].name,
            petObjects[currentPage - 1][i].img
          )
        );
      }
    }
    ...
    function createPopupWindow(name) {
      ...
      let popupInoculations = document.querySelector(
        ".popup__inoculations .popup__note-spec"
      );
      let popupDiseases = document.querySelector(
        ".popup__diseases .popup__note-spec"
      );
      let popupParasites = document.querySelector(
        ".popup__parasites .popup__note-spec"
      );
    ...
    function splitArray() {
      ...
      let petArray = [];
      for (let i = 0; i < 6; i++) {
        allPets = allPets.concat(shuffleArray([0, 1, 2, 3, 4, 5, 6, 7]));
      }
      tempPets = [].concat(allPets);
      while (tempPets.length) {
        let temp = [];
        for (let item of allPets) {
          if (!temp.includes(item)) {
            let index = tempPets.indexOf(item);
            temp.push(item);
            tempPets.splice(index, 1);
            if (temp.length === numberOfCards) {
              allPets = [].concat(tempPets);
              break;
            }
          }
        }
      ...
      for (let object of petArray) {
        let temp = [];
        for (let number of object) {
          temp.push(allPetsInfo[number]);
        }
        petObjects.push(temp);
      }
    }
  });
